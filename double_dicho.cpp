...
void selection_sort(double* array, char** array2, unsigned int size) {
    double tmp;
...
int dichotomie(double* moyennes, int debut, int fin, double moyenne) {
    if(fin - debut == 1) {
        if(fabs(moyennes[fin]-moyenne) <= fabs(moyennes[debut]-moyenne)) {
            return fin;
        } else {
            return debut;
        }
